...
async function run() {
  try {
    ...
    const server = db.collection("saprerecords");
    const serverResults = db.collection("sapreresults");
    
    const user = db.collection('users');
    const datePandL = db.collection("datepandls");
    const allTargets = db.collection("alltargets");
    const serverName = 'sapre';
    let target = 0;
    let ProfitOrLoss = 0;
        ...
        setTimeout(()=>{
           getResult().then(async(resultData)=>{
               ...
               if(win_number === 1 || win_number === 3 || win_number === 7 || win_number === 9){
                ...
                await user.updateMany({currentPeriod:parseInt(period)},[{$set: { wallet: { $add: [ "$wallet", { $arrayElemAt: [ "$sapre", win_number ] },{ $arrayElemAt: [ "$sapre", 10 ] }] } } }]);
                 ...
                 await user.updateMany({currentPeriod:parseInt(period)},[{$set: { wallet: { $add: [ "$wallet", { $arrayElemAt: [ "$sapre", win_number ] },{ $arrayElemAt: [ "$sapre", 12 ] }] } } }]);
                ...
                await user.updateMany({currentPeriod:parseInt(period)},[{ $set: {wallet:{ $add:["$wallet", {$multiply:[{ $arrayElemAt: [ "$sapre", 12 ] },0.75]},{ $arrayElemAt: [ "$sapre", 0] },{ $arrayElemAt: [ "$sapre", 11] }]}}}]);
                ...
                await user.updateMany({currentPeriod:parseInt(period)},[{ $set: {wallet:{ $add:["$wallet", {$multiply:[{ $arrayElemAt: [ "$sapre", 10 ] },0.75]},{ $arrayElemAt: [ "$sapre", 5] },{ $arrayElemAt: [ "$sapre", 11] }]}}}]);
               }
